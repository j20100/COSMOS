...
def class_colors(class_names,class_id, bright=True):
    """
    Generate random colors.
    To get visually distinct colors, generate them in HSV space then
    convert to RGB.
    """
    brightness = 1.0 if bright else 0.7
    hsv = [(float(class_id)/len(class_names), 1, brightness)]
...
'''Added Code'''
def cv_display_mask(image, boxes, masks, class_ids, class_names,
                      scores=None, title="",
                      figsize=(16, 16)):
    # Number of instances
    N = boxes.shape[0]
    ...
    for i in range(N):
        ...
        # Mask
        mask = masks[:, :, i]
        masked_image = apply_mask(no_image, mask, color)


    masked_image.astype(np.uint8)
    return masked_image

def apply_image_masking(image, mask, color, alpha=0.5):
    """Apply the given mask to the image.
    """
    for c in range(3):
        image[:, :, c] = np.where(mask == 1,
...
def cv_img_masked(image, boxes, masks, class_ids, class_names,
                      scores=None, title="",
                      figsize=(16, 16)):
    # Number of instances
    N = boxes.shape[0]
    ...
    for i in range(N):
        ...
        # Mask
        mask = masks[:, :, i]
        masked_image = apply_image_masking(masked_image, mask, color)
...
def apply_depth_image_masking(image, mask, color, alpha=0.5):
    """Apply the given mask to the image.
    """
...
def cv_depth_img_masked(image, boxes, masks, class_ids, class_names,
                      scores=None, title="",
                      figsize=(16, 16)):
    # Number of instances
    N = boxes.shape[0]
